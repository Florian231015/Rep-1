
        const fragen = [
            {
                "frage": "Was ist die Hauptstadt von Deutschland?",
                "optionen": ["Berlin", "Muenchen", "Hamburg", "Koeln"],
                "antwort": "Berlin"
            },
            {
                "frage": "Wer hat die Relativitaetstheorie entwickelt?",
                "optionen": [
                    "Isaac Newton",
                    "Albert Einstein",
                    "Nikola Tesla",
                    "Thomas Edison"
                ],
                "antwort": "Albert Einstein"
            },
            {
                "frage": "Was ist der groesste Planet in unserem Sonnensystem?",
                "optionen": ["Erde", "Mars", "Jupiter", "Saturn"],
                "antwort": "Jupiter"
            },
            {
                "frage": "Wer hat das Buch '1984' geschrieben?",
                "optionen": [
                    "George Orwell",
                    "Aldous Huxley",
                    "J.K. Rowling",
                    "Stephen King"
                ],
                "antwort": "George Orwell"
            },
            {
                "frage": "Was ist die Quadratwurzel von 81?",
                "optionen": ["7", "8", "9", "10"],
                "antwort": "9"
            },
            {
                "frage": "Wer hat das World Wide Web erfunden?",
                "optionen": [
                    "Bill Gates",
                    "Steve Jobs",
                    "Tim Berners-Lee",
                    "Mark Zuckerberg"
                ],
                "antwort": "Tim Berners-Lee"
            },
            {
                "frage": "In welchem Jahr fiel die Berliner Mauer?",
                "optionen": ["1987", "1989", "1991", "1993"],
                "antwort": "1989"
            },
            {
                "frage": "Wer hat das iPhone erfunden?",
                "optionen": ["Microsoft", "Apple", "Samsung", "Nokia"],
                "antwort": "Apple"
            },
            {
                "frage": "Wer hat das Gemaelde 'Die Mona Lisa' gemalt?",
                "optionen": [
                    "Vincent Van Gogh",
                    "Pablo Picasso",
                    "Leonardo da Vinci",
                    "Claude Monet"
                ],
                "antwort": "Leonardo da Vinci"
            },
            {
                "frage": "Was ist der hoechste Berg der Welt?",
                "optionen": ["K2", "Mount Everest", "Kilimandscharo", "Mont Blanc"],
                "antwort": "Mount Everest"
            },
            {
                "frage": "Wer hat das Buch 'Der Herr der Ringe' geschrieben?",
                "optionen": [
                    "J.K. Rowling",
                    "George R.R. Martin",
                    "J.R.R. Tolkien",
                    "Stephen King"
                ],
                "antwort": "J.R.R. Tolkien"
            },
            {
                "frage": "Was ist der kleinste Kontinent der Welt?",
                "optionen": ["Afrika", "Australien", "Antarktis", "Europa"],
                "antwort": "Australien"
            },
            {
                "frage": "Wer hat die Gluehbirne erfunden?",
                "optionen": [
                    "Nikola Tesla",
                    "Thomas Edison",
                    "Alexander Graham Bell",
                    "Benjamin Franklin"
                ],
                "antwort": "Thomas Edison"
            },
            {
                "frage": "In welchem Jahr wurde das Internet oeffentlich zugaenglich?",
                "optionen": ["1983", "1991", "1995", "2000"],
                "antwort": "1991"
            }
        ];
        let aktuelleFrageIndex = 0;
        let punkte = 0;

        document.addEventListener("DOMContentLoaded", () => {
            frageAnzeigen();

            document.getElementById('absenden').addEventListener('click', () => {
                ueberpruefeAntwort();
                aktualisiereButtons();
            });

            document.getElementById('zurueck').addEventListener('click', () => {
                if (aktuelleFrageIndex > 0) {
                    aktuelleFrageIndex--;
                    frageAnzeigen();
                    aktualisiereButtons();
                }
            });

            document.getElementById('weiter').addEventListener('click', () => {
                if (aktuelleFrageIndex < fragen.length - 1) {
                    aktuelleFrageIndex++;
                    frageAnzeigen();
                    aktualisiereButtons();
                }
            });
        });

        function frageAnzeigen() {
            const frageElement = document.getElementById('frage');
            const antwortenElement = document.getElementById('antworten');

            frageElement.innerText = fragen[aktuelleFrageIndex].frage;
            antwortenElement.innerHTML = '';

            fragen[aktuelleFrageIndex].optionen.forEach(option => {
                const div = document.createElement('div');
                const input = document.createElement('input');
                const label = document.createElement('label');

                input.type = 'radio';
                input.name = 'antwort';
                input.value = option;
                label.innerText = option;

                div.appendChild(input);
                div.appendChild(label);
                antwortenElement.appendChild(div);
            });
        }

        function ueberpruefeAntwort() {
            const ausgewaehlteOption = document.querySelector('input[name="antwort"]:checked');
            if (ausgewaehlteOption && ausgewaehlteOption.value === fragen[aktuelleFrageIndex].antwort) {
                punkte++;
                document.getElementById('punkte').innerText = `Punkte: ${punkte} / ${fragen.length}`;
            } else {
                alert('Falsche Antwort!');
            }

            if (aktuelleFrageIndex < fragen.length - 1) {
                aktuelleFrageIndex++;
                frageAnzeigen();
            } else {
                alert('Quiz beendet! Deine Punktzahl: ' + punkte);
                document.getElementById('weiter').disabled = true;
                document.getElementById('absenden').disabled = true;
            }
        }

        function aktualisiereButtons() {
            document.getElementById('zurueck').disabled = aktuelleFrageIndex === 0;
            document.getElementById('weiter').disabled = aktuelleFrageIndex >= fragen.length - 1;
        }
   